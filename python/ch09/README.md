...
이 저장소의 다른 챕터(ch01~ch08, ch10)에는 **Jupyter 노트북(.ipynb)** 파일이 추가되었습니다:
- **한글 설명**: 상세한 개념 설명 포함
- **Ollama 변환**: OpenAI API 대신 무료 로컬 LLM(Ollama)을 사용하도록 변경
- **Colab 호환**: Google Colab에서 실행 가능한 환경 설정 셀 포함

### 일부 챕터에 노트북 파일이 없는 이유

**ch09는 LangGraph Cloud 배포를 위한 프로젝트 구조**입니다:

```
ch09/
├── .langgraph_api/    # LangGraph API 설정
├── a.introductions/   # 배포 소개 예제
├── b.deployment/      # 실제 배포 코드 (src/, langgraph.json 등)
└── README.md
```

이 코드들은 단순 Python 스크립트가 아니라 **LangGraph Cloud에 배포하기 위한 프로젝트 구조**입니다. 다음과 같은 이유로 Colab 노트북 변환이 어렵습니다:

1. **외부 서비스 의존성**: Supabase, LangSmith 등 외부 서비스 연동 필수
2. **프로젝트 구조**: `langgraph.json`, `pyproject.toml` 등 프로젝트 설정 파일 기반
3. **서버 실행 필요**: `langgraph dev` 명령어로 로컬 서버를 실행해야 함
4. **배포 중심**: 클라우드 배포가 목적이므로 단독 실행 불가
...
하지만 다음 파일들은 노트북으로 변환되지 않았습니다:
- `agent_sql_graph.py`, `rag_graph.py`, `search_graph.py`
- `agent_evaluation_rag.py`, `create_rag_dataset.py` 등

**이유**: 이 파일들은 **LangSmith 연동이 필수**입니다. LangSmith는 LangChain의 평가/모니터링 플랫폼으로, API 키와 프로젝트 설정이 필요합니다. Colab에서 단독 실행이 어려우며, 실제 평가를 수행하려면 LangSmith 계정과 설정이 필요합니다.

### 권장 학습 방법

| 챕터 | 학습 방법 |
|------|----------|
| ch01~ch08 | Colab에서 노트북 실행 |
| ch09 | 로컬 환경에서 배포 실습 (위 가이드 참조) |
